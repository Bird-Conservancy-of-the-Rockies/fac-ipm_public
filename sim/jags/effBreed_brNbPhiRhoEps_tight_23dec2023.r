...
####################################################################
## Priors:
br.phi.mn[si] ~ dbeta(27,3)  ## beta is further from symmetrical, but realistic way to apply a prior. 
phi.int[1,si] <- logit(br.phi.mn[si])  #~ dnorm(phi.int.true[1], (phi.int.true[1]/3)^(-2))   ## CV = 0.33

nb.phi.mn[si] ~ dbeta(24,8)  ## beta is further from symmetrical, but realistic way to apply a prior. 
...
br.eps.mn[si] ~ dbeta(9, 21)
eps.int[1,si] <- logit(br.eps.mn[si])

eps.int[2,si] ~ dnorm(0,logit.tau) #T(,0)   ## ADDED 20 Jun 2022
...
#  z.br[ce,si] ~ dbern(ilogit(psi.int[1,si]))
 ...
 ###  number of effective (male) breeders:
